...
def list_my_recipes(user_id: int, cur: sqlite3.Cursor = Depends(get_db)):
...
def submit_recipe(user_id: int, body: RecipeSubmitRequest, cur: sqlite3.Cursor = Depends(get_db)):
    _require_user(cur, user_id)
...
def get_recipe_detail(recipe_id: int, user_id: int, cur: sqlite3.Cursor = Depends(get_db)):
    _require_user(cur, user_id)
...
def update_recipe(recipe_id: int, user_id: int, body: RecipeSubmitRequest, cur: sqlite3.Cursor = Depends(get_db)):
    _require_user(cur, user_id)
...
def delete_recipe(recipe_id: int, user_id: int, cur: sqlite3.Cursor = Depends(get_db)):
    _require_user(cur, user_id)
