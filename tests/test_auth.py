...
def test_signup_then_login_succeeds(client):
    signup_res = client.post("/auth/signup", json={"username": "u_auth_1", "password": "pw12345"})
    assert signup_res.status_code == 200
    user_id = signup_res.json()["user_id"]

    login_res = client.post("/auth/login", json={"username": "u_auth_1", "password": "pw12345"})
    assert login_res.status_code == 200
    assert login_res.json()["user_id"] == user_id


def test_duplicate_signup_returns_409(client):
    client.post("/auth/signup", json={"username": "u_auth_2", "password": "pw12345"})
    res = client.post("/auth/signup", json={"username": "u_auth_2", "password": "다른비번"})
